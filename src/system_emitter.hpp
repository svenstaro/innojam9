...
class EmitterSystem : public entityx::System<EmitterSystem> {
  public:
    EmitterSystem(Game *game, std::function<glm::vec2(glm::vec2, float, float)> p, double cd, double ao)
        : m_game(game), m_path(p), m_cooldown(cd) //, m_angle_offset(ao)
    {
    }
    ...
    void update(entityx::EntityManager &es, entityx::EventManager &events, double dt) {
        ...
        if (m_last_spawned > m_cooldown) {
            ...
            for(int i = 0; i < m_shots_per_cooldown; i++)
            {
                ...
                next.assign<Path>(sin_path,
                                  glm::vec2(1, glm::radians(m_total_elapsed * m_rotation_speed +
                                                            (360.f / m_shots_per_cooldown) * i)),
                                  20.f);
